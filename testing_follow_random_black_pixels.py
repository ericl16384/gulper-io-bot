...
game_region = (0, 0, SCREEN_SIZE[0], SCREEN_SIZE[1])
...
def main():
    ...
    while True:
        ...
        pixels = list(screenshot_gray.getdata())

        black_pixel_coords = []
        for i, pixel in enumerate(pixels):
            if pixel == 0:
                black_pixel_coords.append((i % screenshot.width, i // screenshot.width))
            
            # if len(black_pixel_coords) >= 100:
            #     break
        
        # print(black_pixel_coords)
        # print(len(black_pixel_coords))
        ...
        if black_pixel_coords:
            mouse_pos = random.choice(black_pixel_coords)
